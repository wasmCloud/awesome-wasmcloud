...
// `read_system_conf` reads /etc/resolv.conf, which a Wasm sandbox has no
// equivalent of; hickory does not compile it for wasm at all.
...
async fn lookup_srv(
    ...
    let (resolver_config, resolver_opts) = match dns_config {
        ...
        // On wasm there is no system resolver configuration to inherit, so the
        // nameserver has to be given explicitly via the connection string's
        // `dns_nameserver` option.
        #[cfg(target_family = "wasm")]
        None => {
            let mut group = NameServerConfigGroup::with_capacity(2);
            let ns: SocketAddr = "8.8.8.8:53".parse().expect("literal nameserver address");
            group.push(NameServerConfig::new(ns, Protocol::Udp));
            group.push(NameServerConfig::new(ns, Protocol::Tcp));
            (
