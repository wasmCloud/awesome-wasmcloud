...
- [couchbase-wasi-sockets-p3](host-plugins/component/couchbase-wasi-sockets-p3/) (hosted): A third implementation of `wasmcloud:couchbase@0.2.0` that speaks Couchbase's KV binary protocol directly over `wasi:sockets@0.3.0`, with no SDK and no gateway in front of the cluster. Every socket operation is awaited, so concurrent calls interleave rather than queueing behind one another.
- [couchbase-kv-sdk](host-plugins/component/couchbase-kv-sdk/) (hosted): A second implementation of the same `wasmcloud:couchbase@0.2.0` interface, embedding the official Couchbase Rust SDK to reach a cluster over `couchbases://` instead of the Data API. Serves the locking and `preserve-expiry` operations the Data API cannot, and needs no gateway in front of the cluster; its calls serialize, because the SDK runs on a Tokio runtime under `block_on`.
